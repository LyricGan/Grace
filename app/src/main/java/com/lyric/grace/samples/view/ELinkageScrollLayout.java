package com.lyric.grace.samples.view;
...
public class ELinkageScrollLayout extends ViewGroup implements NestedScrollingParent {
    ...
    public void scrollTo(int x, int y, boolean isInertial) {
        ...
        if (isInertial) {
            ...
            if (scrollY == edge) {
                ...
                if (target != null) {
                    getLinkageScrollHandler(target).flingContent(target, velocity);
                }
    ...
    private int getNextEdge() {
        ...
        if (mFlingOrientation == FLING_ORIENTATION_UP) {
            ...
            for (int i = 0; i < count; i++) {
                ...
                if (topEdge <= scrollY || !isTargetScrollable(target)
    ...
    private boolean isTargetScrollable(View target) {
        LinkageScrollHandler linkageScrollHandler = getLinkageScrollHandler(target);
